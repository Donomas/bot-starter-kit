import {CallbackQueryContext, InlineKeyboardBuilder, MessageContext} from "puregram";
...
export function generateRandomCode(length: number = 20): string {
    let result = '';
    let text: string | string[] = 'ЙЦУКЕНГШЩЗХЪФЫВАПРОЛДЖЭЯЧСМИТЬБЮйцукенгшщзхъфывапролджэячсмитьбюQWERTYUIOPASDFGHJKLZXCVBNMqwertyuiopasdfghjklzxcvbnm0123456789';
    text = text.split('');
 ...
 */
export function separateNumberByDigits(num: string | number): string {
 ...
 */
export function decl(n: number, titles: string[]): string {
 ...
 */
export function shortenLargeNum(num: number, digits: number = 1): string {
    if (num < 1000) {
        return num.toString();
    }
 ...
 */
export async function sendMessage(context: ContextScene, text: string, keyboard: InlineKeyboardBuilder) {
 ...
 */
export async function deleteMessage(context: ContextScene) {
    try {
