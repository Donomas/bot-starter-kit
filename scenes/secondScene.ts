...
import { CallbackQueryContext, DocumentAttachment, InlineKeyboardBuilder, MessageContext } from "puregram";
...
/**
 * @typedef {StepContext<Record<string, any>> & (MessageContext | CallbackQueryContext)} ContextScene
 * @description Объединенный тип контекста для обработки сообщений и колбек-запросов.
 */
...
const callbackHandler = async (context: CallbackQueryContext & StepContext<Record<string, any>>) => {
    ...
    const commandHandlers: Record<string, any> = {
        ...
        comebackStep: async (step: string) => context.scene.step.go(Number(step)),
        ...
        sliderData: async (direction: number) => {
        ...
        deleteDataConfirm: async (name: string) => {
            if (!checkExistenceData(name)) {
                await context.answerCallbackQuery({
                    text: '❌ Базы с таким именем не существует.',
                    show_alert: true
    ...
    const pattern = /^(\w+):?(.+?)?$/i
    const match = (<string>context.data)?.match(pattern);
 ...
 */
function checkExistenceData(name: string): boolean {
 ...
 */
function generateTextSlider(chunk: number, page: number): string {
    let text = '\n';
 ...
 */
function generateKeyboardSlider(chunk: number, page: number): InlineKeyboardBuilder {
 ...
 */
function generateKeyboard(name: string): InlineKeyboardBuilder {
    let keyboard = new InlineKeyboardBuilder();
 ...
 */
function generateKeyboardConfirm(name: string): InlineKeyboardBuilder {
    let keyboard = new InlineKeyboardBuilder();
 ...
 */
function removeDuplicates(objectArray: typeof array, filterArray: string[]): string[] {
    let set1 = new Set();
    ...
    steps: [
        ...
        async (context: ContextScene) => {
            ...
            if (context.mediaGroup) {
                documents = context.mediaGroup.attachments.filter(item => item instanceof DocumentAttachment && item.mimeType?.includes('text/plain')) as DocumentAttachment[];
            ...
            try {
                const usernamesArrays: string[][] = await Promise.all(promises);
                console.log(usernamesArrays);

                // Объединяем все массивы usernames в один
                const allUsernames: string[] = ([] as string[]).concat(...usernamesArrays);
